...
use crate::{
    ...
    error::ErrorCode,
    state::{
        GlobalConfig, ProviderAccount, ProviderStatus, ReceiptSettlement,
        ReceiptSettlementStatus, RewardAccount, RewardAccountKind, RECEIPT_SETTLEMENT_SPACE,
        REWARD_ACCOUNT_SPACE,
    },
    utils::{
        calculate_bps_amount, calculate_claw_amount, checked_add_i128, checked_add_u64,
        checked_sub_u64, initialize_reward_account, require_attestation_caller,
        require_token_mint, require_token_owner, validate_supported_receipt_charge,
    },
};
...
pub fn record_mining_from_receipt(
    ...
    ctx.accounts.user_reward_account.pending_claw_total = checked_add_u64(
        ctx.accounts.user_reward_account.pending_claw_total,
        user_claw,
    )?;
    ctx.accounts.user_reward_account.updated_at = now;

    let provider_debt_offset = if provider.claw_net_position < 0 {
        let provider_debt: u128 = provider.claw_net_position.unsigned_abs();
        provider_debt.min(u128::from(provider_claw_total)) as u64
    } else {
        0
    };
    let provider_locked_claw = checked_sub_u64(provider_claw_total, provider_debt_offset)?;
    if provider_locked_claw > 0 {
        ctx.accounts.provider_reward_account.pending_claw_total = checked_add_u64(
            ctx.accounts.provider_reward_account.pending_claw_total,
            provider_locked_claw,
        )?;
        ctx.accounts.provider_reward_account.updated_at = now;
    }
    provider.claw_net_position = checked_add_i128(
        provider.claw_net_position,
        i128::from(provider_claw_total),
    )?;
...
pub fn settle_finalized_receipt(
    ...
    let provider_reward_account = &mut ctx.accounts.provider_reward_account;

    let now = Clock::get()?.unix_timestamp;

    user_reward_account.pending_claw_total =
        checked_sub_u64(user_reward_account.pending_claw_total, settlement.claw_to_user)?;
    user_reward_account.locked_claw_total =
        checked_add_u64(user_reward_account.locked_claw_total, settlement.claw_to_user)?;
    user_reward_account.updated_at = now;

    provider_reward_account.pending_claw_total = checked_sub_u64(
        provider_reward_account.pending_claw_total,
        settlement.claw_to_provider_locked,
    )?;
    provider_reward_account.locked_claw_total = checked_add_u64(
        provider_reward_account.locked_claw_total,
        settlement.claw_to_provider_locked,
    )?;
    provider_reward_account.updated_at = now;
...
pub struct RecordMiningFromReceipt<'info> {
    ...
    #[account(
        mut,
        seeds = [PROVIDER_SEED, provider_account.provider_wallet.as_ref()],
        bump,
    )]
    pub provider_account: Box<Account<'info, ProviderAccount>>,
    #[account(
        mut,
        seeds = [PROVIDER_REWARD_SEED, provider_account.provider_wallet.as_ref()],
        bump,
    )]
...
pub struct SettleFinalizedReceipt<'info> {
    ...
    #[account(
        mut,
        seeds = [PROVIDER_REWARD_SEED, receipt_settlement.provider_wallet.as_ref()],
        bump,
    )]
    pub provider_reward_account: Box<Account<'info, RewardAccount>>,
    #[account(
        mut,
        seeds = [USER_REWARD_SEED, receipt_settlement.payer_user.as_ref()],
        bump,
    )]
    pub user_reward_account: Box<Account<'info, RewardAccount>>,
    #[account(
