...
use crate::{
    ...
    state::{
        ...
        ReceiptSettlementStatus, RewardAccount, CHALLENGE_BOND_RECORD_SPACE,
    },
    utils::{
        calculate_bps_amount, checked_add_u64, checked_sub_i128, checked_sub_u64,
...
pub fn resolve_challenge_economics(
    ...
    match resolution_code {
        ...
        | ATTESTATION_RESOLUTION_SIGNER_REVOKED => {
            ...
            provider.claw_net_position = checked_sub_i128(
                provider.claw_net_position,
                i128::from(settlement.claw_to_provider_total),
            )?;
            provider.claw_net_position = checked_sub_i128(
                provider.claw_net_position,
                i128::from(record.slash_claw_amount_snapshot),
            )?;
...
pub struct ResolveChallengeEconomics<'info> {
    ...
    #[account(
        mut,
        seeds = [crate::constants::PROVIDER_REWARD_SEED, receipt_settlement.provider_wallet.as_ref()],
        bump,
    )]
    pub provider_reward_account: Box<Account<'info, RewardAccount>>,
    #[account(
        mut,
        seeds = [crate::constants::USER_REWARD_SEED, receipt_settlement.payer_user.as_ref()],
        bump,
    )]
