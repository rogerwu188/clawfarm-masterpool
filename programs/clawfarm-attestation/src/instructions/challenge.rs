...
pub fn resolve_challenge(ctx: Context<ResolveChallenge>, resolution_code: u8) -> Result<()> {
    ...
    let challenge_type = ctx.accounts.challenge.challenge_type;
...
pub fn timeout_reject_challenge(ctx: Context<TimeoutRejectChallenge>) -> Result<()> {
    let timeout_deadline = ctx
        .accounts
        .challenge
        .opened_at
        .checked_add(ctx.accounts.config.challenge_resolution_timeout_seconds)
        .ok_or_else(|| error!(ErrorCode::MathOverflow))?;
    require!(
        Clock::get()?.unix_timestamp > timeout_deadline,
        ErrorCode::ChallengeResolutionTimeoutOpen
    );
    ...
    challenge.resolution_code = ResolutionCode::Rejected as u8;
    let now = Clock::get()?.unix_timestamp;
    challenge.resolved_at = now;
    challenge.status = ChallengeStatus::Rejected as u8;
    ...
    emit!(ChallengeResolved {
        ...
        challenger,
        challenge_type,
        resolution_code: ResolutionCode::Rejected as u8,
    });
    Ok(())
}
...
#[derive(Accounts)]
pub struct TimeoutRejectChallenge<'info> {
